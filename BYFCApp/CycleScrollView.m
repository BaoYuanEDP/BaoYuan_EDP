...
- (void)refreshScrollView
{
    ...
    for (int i = 0; i < 3; i++) {
        _curImageView = [[UIImageView alloc] initWithFrame:scrollView.frame];

                 _curImageView.userInteractionEnabled = YES;
        //imageView.frame = CGRectMake(0, 0, scrollView.bounds.size.width, scrollView.bounds.size.height);
         _curImageView.image = [curImages objectAtIndex:i];
       
        UITapGestureRecognizer *singleTap = [[UITapGestureRecognizer alloc] initWithTarget:self
                                                                                    action:@selector(handleTap:)];
        [ _curImageView addGestureRecognizer:singleTap];
        [singleTap release];
         _curImageView.clipsToBounds = YES;
         _curImageView.contentMode = UIViewContentModeScaleAspectFill|UIViewContentModeScaleAspectFit;
        // 水平滚动
        if(scrollDirection == CycleDirectionLandscape) {
             _curImageView.frame = CGRectOffset( _curImageView.frame, scrollView.frame.size.width * i, 0);
          
            
        }
        // 垂直滚动
        if(scrollDirection == CycleDirectionPortait) {
             _curImageView.frame = CGRectOffset( _curImageView.frame, 0, scrollFrame.size.height * i);
           // imageView.frame = CGRectMake(0, 0, scrollFrame.size.width, scrollFrame.size.height*i);
        }
        
        
        [scrollView addSubview: _curImageView];
    }
...
//    NSInteger pre = [self validPageValue:curPage];
