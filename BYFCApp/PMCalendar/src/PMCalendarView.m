...
//        CGContextFillRect(context, rect);                                                    // /      debug
	...
	NSUInteger numDaysInMonth = [dateOnFirst numberOfDaysInMonth];
        
    //Find number of days in previous month
    NSDate *prevDateOnFirst = [[_currentDate dateByAddingMonths:-1] monthStartDate];
    NSUInteger numDaysInPrevMonth = [prevDateOnFirst numberOfDaysInMonth];
    
    
    //Draw the text for each of those days.
    for(int i = 0; i <= weekdayOfFirst-2; i++) {
        NSUInteger day = numDaysInPrevMonth - weekdayOfFirst + 2 + i;
        
        NSString *string = [NSString stringWithFormat:@"%lu", (unsigned long)day];
